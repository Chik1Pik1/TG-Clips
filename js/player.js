// player.js
import { showNotification, formatTime, throttle, initializeTheme, toggleTheme, initializeTooltips } from './utils.js';
...
function initializePlayer(currentUserId) {
    ...
    function handlePlay() {
        const videoData = videoDataStore[currentVideoIndex];
        if (!hasViewed && currentUserId) {
            videoData.views.add(currentUserId);
            hasViewed = true;
            updateCounters();
        }
    ...
    function downloadCurrentVideo() {
        ...
        if (!videoUrl) {
            showNotification('Нет видео для загрузки!');
            return;
        }
    ...
    function handleReaction(type) {
        ...
        const videoData = videoDataStore[currentVideoIndex];
        if (type === 'like') {
            if (videoData.userLikes.has(currentUserId)) {
                videoData.userLikes.delete(currentUserId);
                videoData.likes--;
            } else {
                if (videoData.userDislikes.has(currentUserId)) {
                    videoData.userDislikes.delete(currentUserId);
                    videoData.dislikes--;
                }
                videoData.userLikes.add(currentUserId);
                videoData.likes++;
                showReaction('like');
            }
            ...
            if (videoData.userDislikes.has(currentUserId)) {
                ...
                videoData.userDislikes.add(currentUserId);
                videoData.dislikes++;
    ...
    function updateChat() {
        ...
        videoData.chatMessages.forEach(msg => {
            ...
            messageEl.className = `message ${msg.sender === currentUserId ? 'sent' : 'received'}`;
    ...
    function sendChat() {
        ...
        if (text) {
            videoData.chatMessages.push({ sender: currentUserId, text });
            chatInput.value = '';
            updateChat();
            updateVideoCache(currentVideoIndex);
            setTimeout(() => {
    ...
    function shareViaTelegram() {
        ...
        if (tg.isVersionGte('6.0')) {
            ...
            showNotification('Имитация: Отправлено в Telegram: ' + text);
        }
    ...
    function publishVideo() {
        if (uploadedFileUrl) {
            ...
            if (channels[currentUserId]) {
                channels[currentUserId].videos.push(uploadedFileUrl);
